...
#include <emb-stdio/Font8x16.h>		// Provides the 8x16 bitmap font for console 
#include <emb-stdio/windows.h>		// This units header
...
unsigned int extDCcount;
INTDC extDC[MAX_EXT_DC];
					 ...
					 int(*prn_handler) (const char* fmt, ...))		// Print handler to display setting result on if successful
{
	uint32_t buffer[23];
	extDCcount = 0;
	memset(extDC, 0, sizeof(INTDC) * MAX_EXT_DC);
	if ((Width == 0) || (Height == 0)) {							// Has auto width or height been requested
		if (mbox_request(&buffer[0], 5,
	...
	if (Depth == 0) {												// Has auto colour depth been requested
		if (mbox_request(&buffer[0], 4,
			...
			if (prn_handler) prn_handler("Failed 2\n");
			return false;										// For some reason get screen physical failed
		}
	}
	if (!mbox_request(&buffer[0], 23,
		...
		MAILBOX_TAG_GET_PITCH, 4, 0, 0)) {
			if (prn_handler) prn_handler ("Failed 3\n");
			return false;
		}												// The requesting settings failed so return the failure


	WINAPI_CB.fb = (uintptr_t)mmu_gpu_to_arm_addr(buffer[17]);					// Transfer the frame buffer
